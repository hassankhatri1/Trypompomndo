...
async def send_plan(q, context, plan_id):
    ...
    await answer(q)
    if p["category"] == "choose" and p["image_file_id"]:
        try:
            await context.bot.send_photo(q.message.chat_id, p["image_file_id"], caption=bold(esc(p["name"])), parse_mode=ParseMode.HTML)
        except TelegramError:
            pass
