...
PLAN_NAME, PLAN_PRICE, UPI_ID, CHANNEL_LINK, DEMO_MEDIA, WELCOME_IMAGE, WELCOME_TEXT, BROADCAST_CONTENT, REJECTION_REASON, EDIT_PLAN_NAME, EDIT_PLAN_PRICE, DB_IMPORT, PLAN_IMAGE, PREMIUM_MENU_IMAGE = range(14)
...
def admin_main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [style_button("🖼️ PREMIUM IMAGE", "admin:premium_image", style="success"),
...
async def send_category_plans(bot, chat_id, category="choose", admin_view=False):
    # CHOOSE and PRO are separate categories.
    # User view: send ONE common image for the whole category, then show all
    # plan buttons underneath it (instead of sending one image per plan).
    heading = (
        "══════« PRO PLAN »═════"
        if category == "pro"
        else "══════« CHOSE PLAN ✅»═════"
    ...
    # USER VIEW -------------------------------------------------------------
    # Use one configured plan image as the common category image. This lets
    # the admin keep the existing per-plan IMAGE setup while the user sees
    # only one image for the entire category.
    common_image = next(
        (p["image_file_id"] for p in plans if p["image_file_id"]),
        None,
    )

    # Heading + single common image.
    await bot.send_message(
        chat_id,
        bold(heading),
        parse_mode=ParseMode.HTML,
    ...
    plan_rows = []
    for plan in plans:
        plan_rows.append([style_button(
            f"💎 {plan['name']}  •  ₹{plan['price']:g}",
            f"user:plan:{plan['id']}",
            style="primary",
        )])
    ...
    # Fallback if no common image is configured or Telegram rejects the file_id.
    await bot.send_message(
        chat_id,
        caption,
        parse_mode=ParseMode.HTML,
...
async def show_all_plans(bot, chat_id):
    # GET PREMIUM = ONE image for the complete plan menu.
    # Do NOT send one image per plan or one image per category.
    # All enabled CHOOSE + PRO plans are placed under the same image.
    choose_plans = get_plans("choose")
    pro_plans = get_plans("pro")
    plans = choose_plans + pro_plans

    if not plans:
        await bot.send_message(
            chat_id,
            bold("💎 <b>Choose Your Plan</b>\n\nNo plans available."),
            parse_mode=ParseMode.HTML,
        )
        return

    # GET PREMIUM uses ONE dedicated admin-controlled image.
    # Per-plan images are intentionally ignored here.
    common_image = get_setting("premium_menu_image")

    # ONE button per plan, all below the ONE image.
    rows = []
    for plan in plans:
        rows.append([style_button(
            f"💎 {plan['name']}  •  ₹{plan['price']:g}",
            f"user:plan:{plan['id']}",
            style="primary",
        )])
...
def admin_plan_menu():
    ...
    for category, label in (("choose", "══════« CHOSE PLAN »═════"), ("pro", "══════« PRO PLAN »═════")):
        ...
        for p in category_plans:
            ...
            if category == "choose":
                buttons.append([
                    style_button("🖼️ IMAGE", f"admin:plan:image:{p['id']}", style="primary"),
...
async def plan_price(update, context):
    ...
    if category == "choose":
        ...
        set_manual_state(context, PLAN_IMAGE)
        await update.message.reply_text(
            bold("🖼️ Send Choose Plan Image\n\nSend an image for this plan, or send SKIP to continue without an image."),
            parse_mode=ParseMode.HTML,
        )
        return PLAN_IMAGE
    ...
    con = db()
    try:
        con.execute("""INSERT INTO plans(name,price,validity_days,description,link,image_file_id,enabled,category)
                       VALUES(?,?,?,?,?,?,1,?)""",
                    (name, price, LIFETIME_DAYS, "Lifetime", "", None, category))
        con.commit()
    finally:
        con.close()
    context.user_data.clear(); clear_manual_state(context)
    await update.message.reply_text(
        bold("ADMIN PLAN CREATED SUCCESSFULLY\n\n"
             f"PLAN: {esc(name)}\nPRICE: ₹{price:g}\nVALIDITY: Lifetime\nCATEGORY: PRO PLAN"),
...
async def plan_image(update, context):
    if not admin_only(update):
        return ConversationHandler.END
    if context.user_data.get("plan_image_id"):
        plan_id = context.user_data["plan_image_id"]
        if update.message and (update.message.text or "").strip().lower() == "remove":
            image_file_id = None
        elif update.message and update.message.photo:
            image_file_id = update.message.photo[-1].file_id
        else:
            await update.message.reply_text(bold("❌ Please send an image or REMOVE."), parse_mode=ParseMode.HTML)
            return PLAN_IMAGE
        con = db()
        try:
            con.execute("UPDATE plans SET image_file_id=? WHERE id=?", (image_file_id, plan_id))
            con.commit()
            plan = con.execute("SELECT name FROM plans WHERE id=?", (plan_id,)).fetchone()
        finally:
            con.close()
        context.user_data.clear(); clear_manual_state(context)
        await update.message.reply_text(bold(f"✅ IMAGE UPDATED FOR: {esc(plan['name'])}"), parse_mode=ParseMode.HTML, reply_markup=admin_plan_menu())
        return ConversationHandler.END
    name = context.user_data.get("plan_name")
    ...
    if update.message and update.message.photo:
        image_file_id = update.message.photo[-1].file_id
    elif update.message and (update.message.text or "").strip().lower() == "skip":
        image_file_id = None
    else:
        await update.message.reply_text(bold("❌ Please send an image or /skip."), parse_mode=ParseMode.HTML)
        return PLAN_IMAGE
    con = db()
    try:
        con.execute("""INSERT INTO plans(name,price,validity_days,description,link,image_file_id,enabled,category)
                       VALUES(?,?,?,?,?,?,1,?)""",
                    (name, price, LIFETIME_DAYS, "Lifetime", "", image_file_id, category))
        con.commit()
    finally:
        con.close()
    context.user_data.clear()
...
async def plan_image_start(q, context):
    if not admin_only(q):
        return ConversationHandler.END
    plan_id = int(q.data.rsplit(":", 1)[1])
    con = db()
    ...
    if not plan:
        ...
        return ConversationHandler.END
    context.user_data.clear()
    context.user_data["plan_image_id"] = plan_id
    set_manual_state(context, PLAN_IMAGE)
    await answer(q)
    await q.message.reply_text(bold(f"🖼️ Send image for: {esc(plan['name'])}\n\nSend REMOVE to remove the current image."), parse_mode=ParseMode.HTML)
    return PLAN_IMAGE
...
async def premium_image_menu(q):
    if not admin_only(q):
        return
    await answer(q)
    image = get_setting("premium_menu_image")
    status = "✅ Set" if image else "❌ Not set"

    keyboard = InlineKeyboardMarkup([
        [style_button("🖼️ SET / CHANGE IMAGE", "admin:premium_image:set", style="success")],
        [style_button("👁️ PREVIEW", "admin:premium_image:preview", style="primary")],
        [style_button("🗑️ REMOVE IMAGE", "admin:premium_image:remove", style="danger")],
        [style_button("🔙 Back", "admin:menu", style="primary")],
    ])
    await edit_or_send(
        q,
        f"🖼️ <b>GET PREMIUM IMAGE</b>\n\n"
        f"Status: <b>{status}</b>\n\n"
        "This is ONE common image shown above all Choose + Pro plans.\n"
        "Plan-wise images are not used in GET PREMIUM.",
        keyboard,
    )


async def premium_image_start(q, context):
    if not admin_only(q):
        return ConversationHandler.END
    context.user_data.clear()
    set_manual_state(context, PREMIUM_MENU_IMAGE)
    await answer(q)
    await q.message.reply_text(
        bold(
            "🖼️ Send the ONE image for GET PREMIUM.\n\n"
            "This image will be shown once, with all plan buttons underneath.\n"
            "Send REMOVE to remove the current image."
        ),
        parse_mode=ParseMode.HTML,
    )
    return PREMIUM_MENU_IMAGE


async def premium_image_save(update, context):
    if not admin_only(update):
        return ConversationHandler.END

    message = update.message
    if (message.text or "").strip().upper() == "REMOVE":
        set_setting("premium_menu_image", "")
        clear_manual_state(context)
        await message.reply_text(
            bold("🗑️ GET PREMIUM image removed."),
            parse_mode=ParseMode.HTML,
            reply_markup=admin_main_keyboard(),
        )
        return ConversationHandler.END
    ...
    if not message.photo:
        await message.reply_text(
            bold("❌ Please send an image/photo.\n\n🖼️ Send the GET PREMIUM image:"),
            parse_mode=ParseMode.HTML,
        )
        return PREMIUM_MENU_IMAGE

    file_id = message.photo[-1].file_id
    set_setting("premium_menu_image", file_id)
    clear_manual_state(context)

    await message.reply_text(
        bold("✅ GET PREMIUM common image saved successfully."),
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
            [style_button("👁️ PREVIEW", "admin:premium_image:preview", style="primary")],
            [style_button("🖼️ CHANGE IMAGE", "admin:premium_image:set", style="success")],
            [style_button("🔙 Admin Panel", "admin:menu", style="primary")],
        ]),
    )
    return ConversationHandler.END


async def premium_image_preview(q, context):
    if not admin_only(q):
        return
    await answer(q)
    image = get_setting("premium_menu_image")
    if not image:
        await q.message.reply_text(
            bold("❌ GET PREMIUM image is not set."),
            parse_mode=ParseMode.HTML,
        )
        return

    # Preview the exact user-facing GET PREMIUM layout.
    await show_all_plans(q.get_bot(), q.message.chat_id)


async def premium_image_remove(q):
    if not admin_only(q):
        return
    set_setting("premium_menu_image", "")
    await answer(q, "Image removed")
    await premium_image_menu(q)
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "user:premium":
        ...
        elif data == "admin:premium_image":
            await premium_image_menu(q)
        elif data == "admin:premium_image:set":
            await premium_image_start(q, context)
        elif data == "admin:premium_image:preview":
            await premium_image_preview(q, context)
        elif data == "admin:premium_image:remove":
            await premium_image_remove(q)
            ...
            await admin_plans(q)
        elif data.startswith("admin:plan:image:"):
            await plan_image_start(q, context)
...
async def message_router(update, context):
    ...
    if admin_only(update) and manual is not None:
        ...
        if manual == PLAN_PRICE:
            ...
            return result
        if manual == PLAN_IMAGE:
            result = await plan_image(update, context)
            return result
        if manual == PREMIUM_MENU_IMAGE:
            result = await premium_image_save(update, context)
            return result
