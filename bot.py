...
PLAN_NAME, PLAN_PRICE, UPI_ID, CHANNEL_LINK, DEMO_MEDIA, WELCOME_IMAGE, WELCOME_TEXT, BROADCAST_CONTENT, REJECTION_REASON, EDIT_PLAN_NAME, EDIT_PLAN_PRICE, DB_IMPORT, PLAN_IMAGE = range(13)
...
async def send_category_plans(bot, chat_id, category="choose", admin_view=False):
    ...
    # For users, every plan is now opened as an image card (when an image is
    # configured) with the plan name/price and a button below it. This makes
    # GET PREMIUM -> plan selection visual instead of showing only text buttons.
    heading = (
        "══════« PRO PLAN »═════"
        if category == "pro"
        else "══════« CHOSE PLAN ✅»═════"
    ...
    # Keep the category heading as a separate message.
    await bot.send_message(
        chat_id,
        bold(heading),
        parse_mode=ParseMode.HTML,
    ...
    for i, plan in enumerate(plans):
        ...
        # User side: show the plan image first, then the plan button.
        # If no image was configured, fall back to a normal text plan card.
        caption = (
            f"💎 <b>{esc(plan['name'])}</b>\n\n"
            f"💰 <b>Price:</b> ₹{plan['price']:g}\n"
            "⏳ <b>Validity:</b> Lifetime"
        )
        keyboard = InlineKeyboardMarkup([[plan_button]])
        ...
        if plan["image_file_id"]:
            try:
                ...
                # If the saved Telegram file_id is invalid, show the text card
                # instead of breaking the GET PREMIUM flow.
                pass
